...
def load_dataset(args):
    ...
    #     features = features.clone()  # 避免原地改动
    #
    #     if args.dataset in ["IMDB", "ACM", "AMiner"] and i == 0:
    #         # 设置缺失比例
    #         missing_ratio = 0.1
    #         num_nodes = features.shape[0]
    #         missing_nodes = np.random.choice(num_nodes, int(num_nodes * missing_ratio), replace=False)
    #
    #         # 保存原始特征、缺失索引
    #         true_feature_list.append(features.clone())
    #         missing_nodes_list.append(torch.tensor(missing_nodes, dtype=torch.long))
    #
    #         # 构造伪缺失输入
    #         features[missing_nodes] = 0.0
    #         new_feature_list.append(features)
    #
    #     elif args.dataset == "DBLP" and i == 1:
    #         missing_ratio = 0.1
    #         num_nodes = features.shape[0]
    #         missing_nodes = np.random.choice(num_nodes, int(num_nodes * missing_ratio), replace=False)
    #
    ...
    #     else:
    #         # 其他节点类型：特征置 0，无需补全监督
    #         true_feature_list.append(features)
    #         missing_nodes_list.append(torch.tensor([], dtype=torch.long))
    ...
    ''''''
    # for i in range(len(feature_list)):
    #     if args.dataset in ["IMDB", "ACM","AMiner"]:
    #         if i == 0:
    #             pass
    #         else:
    #             feature_list[i] = torch.zeros((feature_list[i].shape[0], feature_list[i].shape[1]))
    ...
    return feature_list, adjs, lis, lis_t, labels, num_classes, train_idx, val_idx, test_idx, dl
